...
GhostRecon.IgnoredAbilities = {
	[1604] = true,    --眩晕
	[181089] = true,  --Encounter Event，艾杰斯亚学院 克罗兹，碧蓝魔馆 安布雷斯库，英灵殿 奥丁，青龙寺 刘焰心
	[384289] = true,  --红玉新生法地 搭乘
}
...
if ldb then
	...
	function dataObj:OnTooltipShow()
		...
		self:AddLine(L["Left-click to search."])
		self:AddLine(L["Right-click to configure."])
		self:AddLine(L["Alt- or Shift-click to report current target's abilities."])
	end
...
frame:SetScript("OnEvent", function(self, event, timeStamp)
    local whichAddon = event
    OnAddOnLoaded(event, whichAddon)
end)

-- 初始化函数
function GhostRecon:Initialize()
    -- 初始化设置和其他必要的变量
    -- 例如，加载默认设置，注册其他事件等
    -- ...
end
